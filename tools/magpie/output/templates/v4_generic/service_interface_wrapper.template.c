...
-*/
#define /*-?INTERFACE_NAME-*/_DEFAULT_VTABLE { /*-?list_to_string(interface.get_function_names(), ', ', 'service_')-*/ }
...
for ktable_function in interface.get_pagefault_functions():
        full_list.append('%s_discard' % (interface.get_name()))
        full_list.append('%s_discard' % (interface.get_name()))
        full_list.append('service_%s' % (ktable_function.get_name()))
        full_list.append('%s_discard' % (interface.get_name()))
-*/
/*-if full_list != []*/
