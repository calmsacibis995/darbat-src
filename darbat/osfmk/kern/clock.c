	...
	clock_t			*clock)		/* OUT */
{
	if (host == HOST_NULL || clock_id < 0 || clock_id >= clock_count) {
		*clock = CLOCK_NULL;
        printf("host: %p cock_id: %d clock_count: %d\n", host, clock_id, clock_count);
        printf("HOST_NULL = %p\n", HOST_NULL);
        DBG();
	...
	mach_timespec_t	*cur_time)	/* OUT */
{
	if (clock == CLOCK_NULL)
		return (KERN_INVALID_ARGUMENT);
    // TGB_HACK
    printf("clock_get_time returning bogus value\n");
    cur_time->tv_sec = 1337;
    cur_time->tv_nsec = 1337;
    return KERN_SUCCESS;
	...
	mach_timespec_t	new_time)
{
	mach_timespec_t	*clock_time;
    // TGB HACK
    printf("clock_set_time returning bogus value\n");
    (void)clock_time;
    return KERN_SUCCESS;
...
{
    // TGB HACK
    printf("clock_alarm returning bogus value\n");
    return KERN_SUCCESS;
	...
	struct clock_sleep_trap_args *args)
{
    // TGB HACK
    return KERN_SUCCESS;
    printf("clock_sleep_trap returning bogus value\n");
...
{
    // TGB HACK
    printf("clock_sleep_internal returning bogus value\n");
    return KERN_SUCCESS;
	...
	uint64_t			*deadline)
{
	if( interval == 0 )
	{
		printf( "clock_deadline_blah: ra[0] = %p\n", 
			__builtin_return_address(0) );
		printf( "clock_deadline_blah: ra[1] = %p\n", 
			__builtin_return_address(1) );
		printf( "clock_deadline_blah: ra[2] = %p\n", 
			__builtin_return_address(2) );
		printf( "clock_deadline_blah: ra[3] = %p\n", 
			__builtin_return_address(3) );
		printf( "clock_deadline_blah: ra[4] = %p\n", 
...
{
    // TGB HACK
    printf("mk_timebase_info_trap returning bogus value\n");
    return;
	...
	struct mach_wait_until_trap_args	*args)
{
	...
	wresult = assert_wait_deadline( (event_t)mach_wait_until_trap, 
					THREAD_ABORTSAFE, deadline );
	if (wresult == THREAD_WAITING)
	{
		printf( "doing thread block: %lld -> %lld\n",
			mach_absolute_time(), args->deadline);
...
{
    // TGB HACK
    printf("clock_delay_until returning bogus value\n");
    return;
...
{
    // TGB HACK
    printf("delay_for_interval returning bogus value\n");
    return;
...
{
    // TGB HACK
    printf("delay returning bogus value\n");
    return;
...
{
    // TGB HACK
    printf("clock_adjtime returning bogus value\n");
    return;
...
{
    // TGB HACK
    printf("clock_wakeup_calendar returning bogus value\n");
    return;
