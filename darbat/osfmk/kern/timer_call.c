	...
	timer_call_t			call)
{
	...
	//printf( "int _d_c_e: queue is %p\n", queue );

	current = TC(queue_first(queue));

	//printf( "int _d_c_e: current is %p\n", current );

	while (TRUE) {
	//	printf( "qu: %d\n", queue_end(queue, qe(current)) );
	//	printf( "call->deadline: %lld\n", call->deadline );
	...
	uint64_t				deadline)
{
	...
	//printf( "time_call_enter1: caller %p\n", __builtin_return_address(0) );
	_delayed_call_enqueue(queue, call);
