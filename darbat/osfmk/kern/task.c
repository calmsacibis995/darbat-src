...
#include <igcompat/timer.h>    /* bcopy */
#include <DEBUG.h>
...
#include <mach/vm_map.h> // for vm_allocate

#define UTCB_AREA_SIZE (16*4096)
#define UTCB_AREA_BASE (0xB0000000)


static L4_Word_t utcb_size = 0;
...
task_init(void)
{
	 ...
	 */
	/* printf( "doing task_create_internal\n" ); */
	if (task_create_internal(TASK_NULL, FALSE, &kernel_task) != KERN_SUCCESS)
		panic("task_init\n");

	/* printf( "done task_create_internal\n" ); */
	...
	task_t		*child_task)		/* OUT */
{
    ...
    /* printf(">>> task_create_internal called for the %d time, task is %p\n", ncalls, new_task); */
	...
	if (inherit_memory)
    {
        ...
        //pmap_commpage_hack(new_task->map->pmap);
    }
	else
    {
        pmap_t pmap = pmap_create(0);
        // TGB HACK
        //pmap_commpage_hack(pmap);
    ...
    {
        /* printf(">>> Not creating an EAS for the kernel task"); */
        new_task->eas = 0;
        new_task->utcb_base = NULL;
    ...
    {
        /* printf(">>> Creating an EAS for a user task\n"); */

        // Make up the KIP and UTCB
        L4_Fpage_t kip, utcb;
        L4_Word_t addr = UTCB_AREA_BASE + UTCB_AREA_SIZE;
        new_task->utcb_base = (void *)UTCB_AREA_BASE;

        kip = L4_Fpage( (L4_Word_t)addr, 4096);

        utcb = L4_Fpage( (L4_Word_t) new_task->utcb_base, UTCB_AREA_SIZE);

        // Actually create the EAS
        new_task->eas = eas_create(kip, utcb);
    }
...
// Darbat Extensions
void *
task_get_next_utcb_base(task_t task)
{
    void *tmp = task->utcb_base;

    assert(*((uint32_t *)&tmp) < UTCB_AREA_BASE + UTCB_AREA_SIZE);
    
    *((uint32_t *)&(task->utcb_base)) += utcb_size;
    return tmp;
}

eas_ref_t
task_get_eas(task_t task)
{
    return task->eas;
}

void
task_set_eas(task_t task, eas_ref_t eas)
{
    task->eas = eas;
}
