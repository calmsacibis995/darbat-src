...
// XXX: This is slow, need to be made better later

struct thread_db_entry
{
    char used;
    unsigned long l4_tid;
    thread_t mach_tid;
};

struct thread_db_entry thread_db[10];

void thread_register(unsigned long l4_tid, thread_t mach_tid);

void
thread_register(unsigned long l4_tid, thread_t mach_tid)
{
    int i;
    for(i=0;i<10;i++)
    {
        if(thread_db[i].used == 0)
        {
            break;
        }
    }
    ...
    if(i == 10)
    {
        assert(!"TOO MANY THREADS");
    }

    thread_db[i].used = 1;
    thread_db[i].l4_tid = l4_tid;
    thread_db[i].mach_tid = mach_tid;
}

/* XXX: disgusting CEG hack */
thread_t _thread_find( unsigned long );

thread_t 
_thread_find( unsigned long foo )
{
    int i;
    for(i=0;i<10;i++)
    {
        if(thread_db[i].l4_tid == foo)
        {
            return thread_db[i].mach_tid;
        }
    }
    assert(!"couldn't find a thread!");
    return  (thread_t)NULL;
}

struct funnel_lock * thread_get_funnel_lock(void);
struct funnel_lock *
thread_get_funnel_lock(void)
{
    return current_thread()->funnel_lock;
}
...
/* renamed due to libiguana clash */
kern_return_t
mach_thread_create(
	task_t				task,
	thread_t			*new_thread)
...
unsigned long darbat_thread_create(thread_t, void *, void *);
	...
	thread_t				*new_thread)
{
    ...
    void *eip, *esp;
    result = machine_thread_get_eip_esp(flavor, new_state, new_state_count, &eip, &esp);
    if(result != KERN_SUCCESS)
    {
		pset_unlock(task->processor_set);
		task_unlock(task);
		...
		thread_terminate(thread);
		thread_deallocate(thread);
        L4_KDB_Enter("machine_thread_get_eip_esp failed!");
        return result;
    }
	...
	task_unlock(task);
 	...
 	/** XXX; this is the 2nd worst thing ever **/
 	{
 		thread->machine.pcb->iss.eip = (unsigned int) eip;
 		thread->machine.pcb->iss.esp = (unsigned int) esp;
 	}
	
#if 0
	printf("XXX: Skipping thread_create_running, you probably want to enable this\n");
#else
	//	assert(!"doing darbat_thread_create");
#if 0
 	unsigned long tid = darbat_thread_create(thread, eip, esp);
 	
 	thread_register(tid, thread);
	
 	thread->l4_tid.raw = tid;
#endif
#endif
