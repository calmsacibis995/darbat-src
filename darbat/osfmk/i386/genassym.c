...
#define DECLARE_REL(SYM,VAL) \
	__asm("#DEFINITION##define " SYM "\t%c0" : : "n" ((u_int)(VAL)))
	...
	char	**argv)
{
	...
	/* Mutex structure */
	DECLARE_REL("MUTEX_LOCKED",	offsetof(mutex_t *, lck_mtx.lck_mtx_locked));
	DECLARE_REL("MUTEX_WAITERS",offsetof(mutex_t *, lck_mtx.lck_mtx_waiters));
	DECLARE_REL("MUTEX_PROMOTED_PRI",offsetof(mutex_t *, lck_mtx.lck_mtx_pri));
#if	MACH_LDEBUG
	DECLARE_REL("MUTEX_TYPE",	offsetof(mutex_t *, type));
	DECLARE_REL("MUTEX_PC",	offsetof(mutex_t *, pc));
	DECLARE_REL("MUTEX_THREAD",	offsetof(mutex_t *, thread));
	DECLARE("MUTEX_TAG",	MUTEX_TAG);
#endif	/* MACH_LDEBUG */
	DECLARE_REL("MUTEX_IND",	LCK_MTX_TAG_INDIRECT);
	DECLARE_REL("MUTEX_ITAG",	offsetof(lck_mtx_t *, lck_mtx_tag));
	DECLARE_REL("MUTEX_PTR",	offsetof(lck_mtx_t *, lck_mtx_ptr));
