...
#include <mach/mach_vm.h> // TGB: For vm_allocate
#include <igcompat/compat.h>

#include <commpage_hack.h>
...
{
    int		must,cant;
    printf("commpage_stuff_routine called on %p\n", rd->code_address);
...
commpage_populate( void )
{
    ...
    //bcopy(hack_bin_data, commPagePtr, hack_bin_len);
	...
	*/
    ...
    {
        printf("stuff_routine for addr = %p\n", (*rd)->code_address);
