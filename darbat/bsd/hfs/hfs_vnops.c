...
#include <igcompat/compat.h>  /* bcopy, bzero */
...
hfs_vnop_getattr(struct vnop_getattr_args *ap)
{
	...
	if (VATTR_IS_ACTIVE(vap, va_acl)) {
		if ((cp->c_attr.ca_recflags & kHFSHasSecurityMask) == 0) {
			vap->va_acl = (struct kauth_acl *) KAUTH_FILESEC_NONE;
 ...
 */
static int
hfs_vnop_kqfiltremove(struct vnop_kqfilt_remove_args *ap)
#if 0
 ...
 * Update the times on the cnode then do device close.
 */
static int hfsfifo_close(struct vnop_close_args *ap);
static int
hfsfifo_close(ap)
	struct vnop_close_args /* {
		struct vnode *a_vp;
 ...
 */
int hfsfifo_kqfilt_add(struct vnop_kqfilt_add_args *ap);
int
 ...
 */
int hfsfifo_kqfilt_remove(struct vnop_kqfilt_remove_args *ap);
int
