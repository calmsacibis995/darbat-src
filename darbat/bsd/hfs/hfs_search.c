...
/* XXX: this is to replace dodgey pointer arithmetic
 * XXX: use of cast expressions as lvalues is pants
 */
#define PTR_INC( typ, ptr ) ptr = ((char*)ptr) + sizeof( typ )
...
UnpackSearchAttributeBlock( struct hfsmount *hfsmp, struct attrlist	*alist, searchinfospec_t *searchInfo, void *attributeBuffer )
{
	...
	PTR_INC( uint32_t *, attributeBuffer);	/* advance past the size */
	...
	if ( a != 0 ) {
		if ( a & ATTR_CMN_NAME ) {
			...
			if (hfsmp->hfs_flags & HFS_STANDARD) {
				...
				PTR_INC(attrreference_t *, attributeBuffer);
			} else {
				size_t ucslen;
		...
		if ( a & ATTR_CMN_OBJID ) {
			...
			PTR_INC(fsobj_id_t *, attributeBuffer);
		}
		...
		if ( a & ATTR_CMN_CRTIME ) {
            if (is_64_bit) {
                ...
                PTR_INC(struct user_timespec *, attributeBuffer);
            }
            else {
                searchInfo->creationDate = *((struct timespec *)attributeBuffer);
                PTR_INC(struct timespec *, attributeBuffer);
            }
		}
	...
	if ( a != 0 ) {
		if ( a & ATTR_DIR_ENTRYCOUNT ) {
			...
			PTR_INC(u_int32_t *, attributeBuffer);
		}
	}
