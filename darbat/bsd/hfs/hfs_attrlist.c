...
/* left cast value, increment after assign */
#define LCV_INC_ASSIGN( ptr, typ, val )  \
	do {                             \
		typ tmp;                 \
		tmp = ptr;               \
		*tmp = val;              \
...
/* Assign a pointer to a type (passed as pointer) (eg. memcpy) */
#define LCV_ASSIGN( dest, src, typ ) memcpy( dest, src, sizeof( typ ) )

/* increment a pointer by the size of a type */
#define LCV_INC( ptr, typ ) ptr = ((char*)ptr) + sizeof( typ )
...
packvolcommonattr(struct attrblock *abp, struct hfsmount *hfsmp, struct vnode *vp, struct proc *p)
{
	...
	if (ATTR_CMN_DEVID & attr) {
		...
		LCV_INC_ASSIGN( attrbufptr, dev_t*, hfsmp->hfs_raw_dev );
	}
	...
	if (ATTR_CMN_FNDRINFO & attr) {
		...
		LCV_INC( attrbufptr, vcb->vcbFndrInfo );
	}
	if (ATTR_CMN_OWNERID & attr) {
		if (cp->c_uid == UNKNOWNUID)
			// *((uid_t *)attrbufptr)++ = kauth_cred_getuid(proc_ucred(p));
			LCV_INC_ASSIGN( attrbufptr, uid_t*, kauth_cred_getuid(proc_ucred(p)) );
		else
			// *((uid_t *)attrbufptr)++ = cp->c_uid;
			LCV_INC_ASSIGN( attrbufptr, uid_t*, cp->c_uid );
	}
	if (ATTR_CMN_GRPID & attr) {
		// *((gid_t *)attrbufptr)++ = cp->c_gid;
		LCV_INC_ASSIGN( attrbufptr, gid_t *, cp->c_gid );
	}
...
//			(cp->c_uid == UNKNOWNUID) ? cp->c_mode & ~(S_ISUID | S_ISGID) : cp->c_mode;
		LCV_INC_ASSIGN( attrbufptr, uint32_t*, 
				(cp->c_uid == UNKNOWNUID) ? 
	...
	if (ATTR_CMN_NAMEDATTRLIST & attr) {
		...
		LCV_INC( attrbufptr, struct attrreference * );
	}
	if (ATTR_CMN_FLAGS & attr) {
		// *((uint32_t *)attrbufptr)++ = cp->c_flags;
		LCV_INC_ASSIGN( attrbufptr, uint32_t*, cp->c_flags );
	}
	if (ATTR_CMN_USERACCESS & attr) {
		...
		LCV_INC_ASSIGN( attrbufptr, uint32_t*, 
				DerivePermissionSummary(cp->c_uid, 
							cp->c_gid, 
							cp->c_mode,
							VTOVFS(vp),
							kauth_cred_get(),
...
packvolattr(struct attrblock *abp, struct hfsmount *hfsmp, struct vnode *vp)
{
	...
	if (ATTR_VOL_SIGNATURE & attr) {
		...
		LCV_INC_ASSIGN( attrbufptr, uint32_t*, 
				(uint32_t)vcb->vcbSigWord );
		
	}
	if (ATTR_VOL_SIZE & attr) {
		// *((off_t *)attrbufptr)++ =
		//		(off_t)vcb->totalBlocks * (off_t)vcb->blockSize;
		LCV_INC_ASSIGN( attrbufptr, off_t*, 
				(off_t)vcb->totalBlocks 
				* (off_t)vcb->blockSize );
	}
	...
	if (ATTR_VOL_OBJCOUNT & attr) {
		...
		LCV_INC_ASSIGN( attrbufptr, uint32_t*,
				(uint32_t)vcb->vcbFilCnt 
				+ (uint32_t)vcb->vcbDirCnt );
	}
	...
	if (ATTR_VOL_MOUNTPOINT & attr) {
		...
		varbufptr = (char*) varbufptr + attrlength;
	...
	if (ATTR_VOL_NAME & attr) {
		...
		ar->attr_length = cp->c_desc.cd_namelen + 1;
    ...
    if (ATTR_VOL_MOUNTFLAGS & attr) {
	    ...
	    LCV_INC_ASSIGN( attrbufptr, uint32_t*, (uint32_t)vfs_flags(mp) );
    }
	if (ATTR_VOL_MOUNTEDDEVICE & attr) {
		...
		ar->attr_length = strlen(mp->mnt_vfsstat.f_mntfromname) + 1;
	...
	if (ATTR_VOL_ENCODINGSUSED & attr) {
		...
		LCV_INC_ASSIGN( attrbufptr, unsigned long long *, 
				(unsigned long long)vcb->encodingsBitmap );
	}
	if (ATTR_VOL_CAPABILITIES & attr) {
		...
		LCV_INC( attrbufptr, vol_capabilities_attr_t* );
	}
	...
	struct proc *p)
{
	...
	if (ATTR_CMN_OBJTYPE & attr) {
		...
		LCV_INC_ASSIGN( attrbufptr, 
				fsobj_type_t*, IFTOVT(cap->ca_mode) );
	}
	if (ATTR_CMN_OBJTAG & attr) {
		// *((fsobj_tag_t *)attrbufptr)++ = VT_HFS;
		LCV_INC_ASSIGN( attrbufptr, fsobj_tag_t*, VT_HFS );
	}
	/*
	 ...
	 */
    if (ATTR_CMN_OBJID & attr) {
	    fsobj_id_t *oi = attrbufptr;

	    // ((fsobj_id_t *)attrbufptr)->fid_objno = cdp->cd_cnid;
	    oi->fid_objno = cdp->cd_cnid;
	...
	if (ATTR_CMN_PAROBJID & attr) {
		...
		oi->fid_objno = cdp->cd_parentcnid;
	...
	if (ATTR_CMN_SCRIPT & attr) {
		...
		LCV_INC_ASSIGN( attrbufptr, text_encoding_t*, 
				cdp->cd_encoding );
	}
	if (ATTR_CMN_CRTIME & attr) {
	    if (is_64_bit) {
	...
	if (ATTR_CMN_OWNERID & attr) {
		...
		LCV_INC_ASSIGN( attrbufptr, uid_t*, 
				(cap->ca_uid == UNKNOWNUID) 
				? kauth_cred_getuid(proc_ucred(p)) 
				: cap->ca_uid );
	}
	if (ATTR_CMN_GRPID & attr) {
		// *((gid_t *)attrbufptr)++ = cap->ca_gid;
		LCV_INC_ASSIGN( attrbufptr, gid_t*, cap->ca_gid; );
	}
	if (ATTR_CMN_ACCESSMASK & attr) {
		/*
		 * [2856576]  Since we are dynamically changing the owner, also
	...
	if (ATTR_CMN_FLAGS & attr) {
		...
		LCV_INC_ASSIGN( attrbufptr, uint32_t*, cap->ca_flags );
	}
	if (ATTR_CMN_USERACCESS & attr) {
		// *((uint32_t *)attrbufptr)++ =
		...
		//		current_proc());
		LCV_INC_ASSIGN( attrbufptr, uint32_t*, 
				DerivePermissionSummary(cap->ca_uid, 
							cap->ca_gid,
							cap->ca_mode, mp, 
	...
	struct cat_attr * cattrp)
{
	...
	if (ATTR_DIR_LINKCOUNT & attr)
		...
		LCV_INC_ASSIGN( attrbufptr, uint32_t*, cattrp->ca_nlink );
	if (ATTR_DIR_ENTRYCOUNT & attr) {
		...
		LCV_INC_ASSIGN( attrbufptr, uint32_t*, entries );
	}
	...
	struct cat_fork *rsrcfork)
{
	...
	if (ATTR_FILE_FORKCOUNT & attr) {
		...
		LCV_INC_ASSIGN( attrbufptr, uint32_t*, 2 );
	}
