...
#include <DEBUG.h>
/* XXX: broken headers, so we can't include the right thing. CEG */
	...
	off_t embeddedOffset, u_int64_t disksize, struct proc *p, void *args, kauth_cred_t cred)
{
	...
	if (signature == kHFSPlusSigWord) {
		 ...
		 */
		printf( "hfs_mount: bad sig: 0x%x\n", signature );
		printf( "hfs_mount: bad ver? 0x%x\n", version );
		return (EINVAL);
	}
	...
	{
		printf( "trying to mount dirty HFS!\n" );
		return (EINVAL);
	}
	...
	if (   (vcb->vcbAtrb & kHFSVolumeJournaledMask)
		...
		if (retval != 0) {
			...
			printf( "something beirdo in HFS!\n" );
			retval = EINVAL;
			goto ErrorExit;
		} else if (hfsmp->jnl) {
...
/* XXX: put in header */
__private_extern__ int overflow_extents(struct filefork *fp);
			...
			struct cat_attr *fattr, struct cat_fork *forkinfo)
{
	...
	jdesc.cd_nameptr = (char*) name;  /* XXX: drops const! */
...
hfs_late_journal_init(struct hfsmount *hfsmp, HFSPlusVolumeHeader *vhp, void *_args)
{
	...
	if (SWAP_BE32(vhp->journalInfoBlock) != jib_fork.cf_extents[0].startBlock) {
		printf("hfs: The journal_info_block moved (was: %ld; is: %ld).  Fixing up\n",
		       SWAP_BE32(vhp->journalInfoBlock), 
		       (long) (jib_fork.cf_extents[0].startBlock) );
