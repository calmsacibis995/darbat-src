...
/* XXX: should come from headers! */
extern int overflow_extents(struct filefork *fp);
extern int is_file_clean(vnode_t, off_t);
		...
		struct cnode *skip_cp, struct proc *theProcPtr, kauth_cred_t myp_ucred, dev_t dev )
{
       ...
       /* XXX: easier to reference ourself than pull out all the references
	* to this and deps. This is only called from one place and is
	* currently ifdef'd out. CEG
	*/
       (void) do_access_check;
