...
#include <sys/init.h>
#include <igcompat/compat.h> /* bzero, kprintf, bcmp, bcopy */
...
#define DLIL_ALWAYS_DELAY_DETACH 0
	...
	int			raw)
{
	...
	DBG();
			...
			if (proto->kpi.v1.pre_output)
			{
				// printf( "calling pre_output %p\n", 
				//	proto->kpi.v1.pre_output );
				retval = proto->kpi.v1.pre_output(ifp, proto_family, &m, dest, route, frame_type, dst_linkaddr);
				// printf( "pre_output returned %d\n", retval );
			}
		}
...
{
	return dlil_if_attach_with_address(ifp, NULL);
}
