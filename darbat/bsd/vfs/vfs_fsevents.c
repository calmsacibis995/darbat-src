...
int vnode_vttoif(enum vtype); /* FIXME: should be in a header */
...
fsevent_unmount(struct mount *mp)
{
	...
	for(j=0; j < KFS_NUM_ARGS; j++) {
		...
		if (vn_getpath(vp, pathbuff, &pathbuff_len) != 0 || pathbuff[0] == '\0') {
			...
			printf("fsevent_unmount: vn_getpath failed! vp 0x%p vname 0x%p (%s) vparent 0%p\n",
...
fseventsf_read(struct fileproc *fp, struct uio *uio,
		__unused struct ucred *cred, __unused int flags,
...
void fsevents_init(void);  /* this is created from a pseudo-device?? */
