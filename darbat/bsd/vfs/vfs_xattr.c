...
#include <igcompat/compat.h> /* bcmp, bcopy, bzero */
...
shift_data_down(vnode_t xvp, off_t start, size_t len, off_t delta, vfs_context_t context)
{
	...
	for(pos=start+len-chunk; pos >= start; pos-=chunk) {
		...
		if (iolen != 0) {
			printf("xattr:shift_data: error reading data @ %lld (read %d of %ld) (%d)\n",
				pos, ret, (unsigned long)chunk, ret);
			break;
		}
		
		ret = vn_rdwr(UIO_WRITE, xvp, buff, chunk, pos + delta, UIO_SYSSPACE, IO_NODELOCKED, ucred, &iolen, p);
		if (iolen != 0) {
			printf("xattr:shift_data: error writing data @ %lld (wrote %d of %ld) (%d)\n",
				pos+delta, ret, (unsigned long)chunk, ret);
			break;
		}
