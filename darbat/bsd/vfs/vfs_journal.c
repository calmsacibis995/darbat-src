...
buffer_flushed_callback(struct buf *bp, void *arg)
{
    ...
    // that into account.
    OSAddAtomic(bufsize, (SInt32*)&tr->num_flushed);
...
{
    int i, ret, orig_checksum, checksum, max_bsize = 0;
			   ...
			   void         *arg)
{
    ...
    if (phys_blksz > min_fs_blksz) {
		printf("jnl: create: error: phys blksize %d bigger than min fs blksize %lu\n",
			   phys_blksz, (unsigned long)min_fs_blksz);
		return NULL;
    }
...
//XXXdbg    if ((jnl->jhdr->end % jnl->jhdr->jhdr_size) != 0) {
	...
	if (flags & JOURNAL_RESET) {
		printf("jnl: journal start/end pointers reset! (jnl 0x%p; s 0x%llx e 0x%llx)\n",
...
journal_close(journal *jnl)
{
    ...
    if ((jnl->flags & JOURNAL_INVALID) == 0) {
		...
		printf("jnl: close: journal 0x%p, is invalid.  aborting outstanding transactions\n", jnl);
...
journal_modify_block_end(journal *jnl, struct buf *bp)
{
    ...
    memcpy(blkptr, (void*) buf_dataptr(bp), buf_size(bp));
...
end_transaction(transaction *tr, int force_it)
{
	...
	//
	memcpy((void*)&jnl->old_start[0], (void*)&jnl->old_start[1], sizeof(jnl->old_start)-sizeof(jnl->old_start[0]));
    ...
    for(blhdr=tr->blhdr; blhdr; blhdr=next) {
		...
		for(i=1; i < blhdr->num_blocks; i++) {
			...
			if (vp == NULL && lblkno == blkno) {
			    printf("jnl: end_tr: bad news! bp @ 0x%p w/null vp and l/blkno = %qd/%qd.  aborting the transaction (tr 0x%p jnl 0x%p).\n",
			...
			if ((lblkno == blkno) && (vp != jnl->fsdev)) {
				...
				if (VNOP_BLKTOOFF(vp, lblkno, &f_offset)) {
					printf("jnl: end_tr: vnop_blktooff failed @ 0x%p, jnl 0x%p\n", bp, jnl);
					goto bad_journal;
				}
				if (VNOP_BLOCKMAP(vp, f_offset, buf_count(bp), &blkno, &contig_bytes, NULL, 0, NULL)) {
					printf("jnl: end_tr: can't blockmap the bp @ 0x%p, jnl 0x%p\n", bp, jnl);
					goto bad_journal;
				}
				if ((uint32_t)contig_bytes < buf_count(bp)) {
					printf("jnl: end_tr: blk not physically contiguous on disk@ 0x%p, jnl 0x%p\n", bp, jnl);
					goto bad_journal;
				}
    ...
    for(; blhdr; blhdr=next) {
	    ...
	    qsort(&blhdr->binfo[1], blhdr->num_blocks-1, sizeof(block_info), (void*) journal_binfo_cmp);
		...
		for(i=1; i < blhdr->num_blocks; i++) {
			...
			if (errno == 0 && bp != NULL) {
				...
				printf("jnl: end_transaction: could not find block %Ld vp 0x%p!\n",
    ...
    for(blhdr=tr->blhdr; blhdr; blhdr=next) {
		...
		for(i=1; i < blhdr->num_blocks; i++) {
			...
			if (errno == 0) {
				...
				printf("jnl: abort_tr: could not find block %Ld vp 0x%p!\n",
					   blhdr->binfo[i].bnum, blhdr->binfo[i].bp);
				if (bp) {
					buf_brelse(bp);
				}
