...
/* XXX: there's a reason this isn't included :) - CEG */
// #include <ipc/ipc_port.h>  /* ipc_port_copytout_somejunk */
					...
					__unused int *retval)
{
	...
	if((kr = copyin_shared_file((vm_offset_t)mapped_file_addr, 
		...
		if((caller_flags & ALTERNATE_LOAD_SITE) && systemLogDiags) {
			printf("load_shared_file:  Failed to load shared file! error: 0x%x, Base_address: 0x%p, number of mappings: %d, file_control 0x%p\n", error, local_base, map_cnt, file_control);
			for(i=0; i<map_cnt; i++) {
