...
// #if FIXME
		...
		if (delta_size > 0) {
			...
			if (copyout((void*)tmp, map_addr + scp->filesize,
								delta_size)) {
				(void) vm_deallocate(
...
)
{
	...
	if (map_size > 0) {
		...
		if (delta_size > 0) {
			...
			if (copyout((void*)tmp, map_addr + scp64->filesize,
								delta_size)) {
				(void) vm_deallocate(
						kernel_map, tmp, delta_size);
		return (LOAD_FAILURE);
...
)
{
	...
	if (result->thread_count != 0) {
		printf("XXXX: doing mach_trhead_create\n" );
		kret = mach_thread_create(task, &thread);
		if (kret != KERN_SUCCESS)
			return(LOAD_RESOURCE);
		thread_deallocate(thread);
	}

#if 0 // XXX TGB This is an ugly hack and should be fixed to fully support Mach-O
...
)
{
	...
	while (total_size > 0) {
        ...
        thread_setuserstack(thread, *user_stack);
...
)
{
	...
	*entry_point = MACH_VM_MIN_ADDRESS;
	while (total_size > 0) {
        ...
        thread_setentrypoint(thread, *entry_point);
		if (ret != KERN_SUCCESS)
			return(LOAD_FAILURE);
		ts += size;	/* ts is a (unsigned long *) */
	}
