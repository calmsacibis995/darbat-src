...
/* just local */
static int kdbg_read(user_addr_t buffer, size_t *number);
...
kdbg_mapinit(void)
{
	...
	if (tts_mapptr) {
			...
			if (p->task) {
				...
				(void)strncpy((char*)&tts_mapptr[i].task_comm, p->p_comm, sizeof(tts_mapptr[i].task_comm) - 1);
				i++;
			}
		}
		tts_count = i;
	...
	if (kd_mapptr && tts_mapptr)
	  {
		...
		task_act_iterate_wth_args((void*)tts_mapptr[i].task, (void*)kdbg_resolve_map, &akrt);
...
kdbg_control(int *name, u_int namelen, user_addr_t where, size_t *sizep)
{
	...
	switch(name[0]) {
		...
		case KERN_KDSETREG:
			...
			if (copyin((void*)where, (void*) &kd_Reg, sizeof(kd_regtype))) {
				ret= EINVAL;
				break;
			}
