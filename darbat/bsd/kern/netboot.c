		   ...
		   char * * mount_dir, char * * image_path)
{
	...
	*image_path = start;
    ...
    /* this is dodgey, return is const */
    *host = (char*) inet_ntop(AF_INET, iaddr_p, tmp, sizeof(tmp));
    return (TRUE);
}
...
netboot_setup(struct proc * p)
{
	...
	/* FIXME: this first param looks bogus... */
	if (VFS_ROOT((struct mount*)mountlist.tqh_last, &newdp, &context))
...
#if NFSCLIENT /* alternate impl. in bsd_init for no NFS */
int
netboot_root(void)
{
    return (S_netboot);
