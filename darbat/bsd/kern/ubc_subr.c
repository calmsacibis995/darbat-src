...
/* XXX: I don't know what's up with this guy - CEG */
...
/* The Debugger function... somewhere in the bowels of mach */
...
ubc_info_free(struct ubc_info *uip)
{
	...
	credp = (kauth_cred_t) uip->ui_ucred;
	if (credp != NOCRED) {
		uip->ui_ucred = (ucred_t*) NOCRED;
...
{
        if (UBCINFOEXISTS(vp))
	        return (kauth_cred_t) (vp->v_ubcinfo->ui_ucred);
...
ubc_setcred(struct vnode *vp, struct proc *p)
{
	...
	if (credp == NOCRED) {
		uip->ui_ucred = (ucred_t*) kauth_cred_proc_ref(p);
	} 
	vnode_unlock(vp);
