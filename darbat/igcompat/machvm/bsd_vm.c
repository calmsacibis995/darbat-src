...
/* XXX: should be pulled in from ipc_kobject.h via somewhere else,
 * but this stuff is 'being phased out' anyway. CEG.
 */
	...
	struct macx_triggers_args *args)
{
	...
	panic( "need to fix thread_t here" ); /* XXX: ceg */
 ...
 *
 */
void vnode_pager_bootstrap(void);
...
vm_prot_t		protection_required)
{
	...
	PAGER_DEBUG(PAGER_ALL, ("vnode_pager_data_request: %p, %x, %x, %x\n", mem_obj, (int) offset, length, protection_required));

	vnode_object = vnode_pager_lookup(mem_obj);

	PAGER_DEBUG(PAGER_PAGEIN, ("vnode_pager_data_request: %p, %x, %x, %x, vnode_object %p\n", mem_obj, (int) offset, length, protection_required, vnode_object));
