...
#include <igcompat/compat.h> /* PE_parse_boot_arg */
#include <DEBUG.h>
	...
	vm_offset_t		*endp)
{
...
#ifdef DARBAT
	vm_page_buckets = malloc( vm_page_bucket_count 
	...
	printf( "about to pmap_startup\n" );
	pmap_startup(&virtual_space_start, &virtual_space_end);
	printf( "done pmap_startup\n" );
	...
	vm_offset_t *endp)
{
	...
	/* CEG FIXME: looks like we just need pmap_next_page?? */
	...
	int	interruptible )
{
	...
	printf( "vm_page_wait: %d < %d\n", vm_page_free_count, vm_page_free_target );

	assert( !"vm_page_wait - that ain't never gonna happen!" );
	...
	unsigned int	npages)
{
	...
	for (m = NEXT_PAGE(pages); m != VM_PAGE_NULL; m = NEXT_PAGE(m)) {
		if (m->phys_page != prev_addr + 1) {
			printf("m 0x%p prev_addr 0x%x, current addr 0x%x\n",
			       m, prev_addr, m->phys_page);
			printf("pages 0x%p page_count %d\n", pages, page_count);
	...
	if (page_count != npages) {
		printf("pages 0x%p actual count 0x%x but requested 0x%x\n",
