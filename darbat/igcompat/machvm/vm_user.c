	...
	for(i=0; i<count; i++) {
		...
		if(map_size != 0) {
			error = vm_map_copyin(map,
					...
					&copy);
			if (KERN_SUCCESS == error) {
				error = vm_map_copyout(current_map(), 
						&map_addr,
						copy);
				if (KERN_SUCCESS == error) {
					data_list[i].address =
	...
	mach_vm_size_t	*data_size)
{
	...
	if (KERN_SUCCESS == error) {
		error = vm_map_copy_overwrite(current_map(),
...
/* XXX: This is meant to a macro or something? check mach_vm.defs */
#if 0
kern_return_t
vm_region(
	vm_map_t			map,
...
/* XXX: No longer used in the 'wide' model? (mach_vm.defs) */
#if 0
kern_return_t
vm_region_recurse(
	vm_map_t			map,
