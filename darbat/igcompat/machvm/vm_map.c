	...
	vm_map_t	map)
{
	...
	if (map->first_free != entry) {
		printf("Bad first_free for map 0x%p: 0x%p should be 0x%p\n",
...
#define	RETURN(value)	{ result = value; goto BailOut; }
 ...
 StartAgain: ;
	...
	if (anywhere) {
		...
		assert(first_free_is_valid(map));
		if (start == map->min_offset) {
			if( map->first_free == NULL )
			{
				printf( "map is %p\n", map );
				printf( "about to die, %p\n", __builtin_return_address(0) );
	...
	vm_map_offset_t		pmap_addr)
{
    ...
    /*
    printf( "vm_map_wire_nested: map %p, start %x, end %x, map_pmap %p\n",
            map, (int) start, (int) end, map_pmap );
    */
	...
	vm_inherit_t		inheritance)
{
	kern_return_t		result;
	vm_map_entry_t		entry;
	vm_map_entry_t		insp_entry = NULL;
	vm_map_entry_t		new_entry;
	struct vm_map_header	map_header;
	...
	vm_map_t	map)
{
	...
	assert(!"direct thread reference");
	(void) thread;
	...
	/* XXX: this uses the pmap internals directly... */
	
	panic("This won't work in darbat" );
