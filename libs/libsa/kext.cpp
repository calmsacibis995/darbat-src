    ...
    bool * caller_owns_code)
{
    ...
    */
    ...
    if (!extDict) {
#ifdef L4IOKIT
        IOLog("%s: extension \"%s\" cannot be found\n",
           __func__, bundleid);
#else
    ...
    Boolean    skipKernelDependencies)
{
    ...
    if (!keyIterator) {
        // XXX: Add log message
#ifdef L4IOKIT
 	IOLog("can't get key iterator for osbundlelibraries\n");
#endif
        result = false;
        goto finish;
    }
...
#ifdef L4IOKIT
	/*
	 * -gl: It's a void.  V-O-I-D !
	 */
	kfree((void *) kmod_info, sizeof(kmod_info_t));
#else
