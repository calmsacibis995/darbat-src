 ...
 * Include this too
 */
#include <mach/mach_host.h>	/* hostinfo() */
#include <l4/kdebug.h>

// L4IOKit hack.  Currently in KernelConfigTables.cpp
extern const char *gIOHackMods;
#endif
...
#ifdef L4IOKIT
#ifdef VERBOSE_IOKIT
    IOLog("%s: driverInfo %p\n", __func__, (void *)driverInfo);
    IOLog("%s: driverInfo->paddr = %p\n", __func__, (void *)driverInfo->paddr);
#endif
#endif
#if defined (__ppc__)
...
#ifdef L4IOKIT
    dataBuffer->plistAddr = (char *)ml_static_ptovirt((vm_offset_t)dataBuffer->plistAddr);
#else
    dataBuffer->plistAddr = ml_static_ptovirt(dataBuffer->plistAddr);
#endif
    if (dataBuffer->moduleAddr)
#ifdef L4IOKIT
      dataBuffer->moduleAddr = (char *)ml_static_ptovirt((vm_offset_t)dataBuffer->moduleAddr);
#else
...
#ifdef L4IOKIT
#ifdef VERBOSE_IOKIT
    IOLog("%s: driverPlist is at %p, driver is at %p\n",
	__func__, 
	(void *)dataBuffer->plistAddr, 
	(void *)dataBuffer->moduleAddr);
#endif
#endif
...
finish:
    ...
    if (loaded_kmod) {
#ifdef L4IOKIT
        kfree((void *)loaded_kmod, sizeof(kmod_info_t));
#else
...
#ifdef L4IOKIT
    if (loaded_kmod) kfree((void *)loaded_kmod, sizeof(kmod_info_t));
#else
