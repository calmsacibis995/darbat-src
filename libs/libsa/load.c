...
#ifdef L4IOKIT
/*
 * XXX: I would include libkern/libkern.h, except there are some crap in there
 * XXX: that would conflict with our existing defs, so we just pull in the 
 * XXX: cache stuff.
 */
...
#ifndef VERBOSE_IOKIT
kload_log_level log_level = 0;
#else
kload_log_level log_level = 999;
#endif
...
#ifdef VERBOSE_IOKIT
    printf("done __kload_load_modules\n");
#endif
#endif /* not KERNEL */
...
#else
#ifdef L4IOKIT
		// ???? we already have kmod id so it must be loaded (??)
		if (G_current_load_entry &&
		    G_current_load_entry->loaded_address) {
#ifdef VERBOSE_IOKIT
			printf("XXX: not loading module with load addr \n");
#endif
...
#ifdef VERBOSE_IOKIT
	printf("XXX: not loading since do_load flag not set\n");
#endif
            result = kload_error_none;
	    goto finish;
    }
...
#ifdef L4IOKIT
#if 0
   printf("kernel_kmod_info %p, load addr %p, kld_header %p\n",
        kernel_kmod_info, G_current_load_entry->kernel_load_address,
        kld_header);
   /*
    * XXX: Yes, this i gives us the right address, however, the start
    * XXX: is only an offset??  why?  Who tacks on the correct function
    * XXX: address here?  kld_load_from_memory()?
    */
    local_kmod_info->start += round_page(G_current_load_entry->kernel_load_address);
    local_kmod_info->stop += round_page(G_current_load_entry->kernel_load_address);
    printf("XXX: incorrectly appending load address to start\n");
#endif
#endif
...
#ifdef L4IOKIT
#ifdef VERBOSE_IOKIT
 	IOLog("G_current_load_entry not there, returning zero\n");
#endif
#endif
        return 0;
    }
...
#ifdef L4IOKIT
#ifdef VERBOSE_IOKIT
	IOLog("found machO->seg[0].vmaddr, is %p\n", 
	(void *)machO->seg[0].vmaddr);
#endif
#endif
