...
/* XXX: L4IOKIT: added !__ppc__ to follow what's in x86 Mac OS X headers */
 ...
 * XXX L4IOKIT
 *
 * if __APPLE_CC__ is defined, it automatically uses the old abi. i.e.
 * assume -fapple-kext is on.
 * otherwise, it uses new gcc3 abi.
 */
#endif	/*L4IOKIT*/
...
#ifndef __APPLE_CC__
    int	isPointerToMemberFunction = 0;
    size_t	index;
#endif

#ifndef __APPLE_CC__
// Ripped from the GCC doco (I am growing lots of body and chest hair)
// I'm a real man! :)
/* By default, the C++ compiler will use the lowest bit of the pointer
   ...
   field will be used.  */
// apple source code indicates that it should always be set at the PFN ..
// so yeh.
// Ugg .. ifdef hell and c++ abi hell.
#endif
    map.fIn = func;

#ifndef __APPLE_CC__
    isPointerToMemberFunction = (size_t) (map.fptmf3.fPFN) & 0x1UL;
    index = ((size_t)map.fptmf3.fPFN - 1) / 4;	/* XXX is this right?*/
#if 0
    // thank goodness our panic is so permissive
    panic("isPointerToMemberFunction = %d index = %d\n",
	isPointerToMemberFunction, index);
#endif
#endif
#ifdef __APPLE_CC__
    if (map.fptmf2.fToff)
...
#else
    else if (isPointerToMemberFunction == 0) {
#endif
	// Not virtual, i.e. plain member func
#ifdef __APPLE_CC__
	return map.fptmf2.u.fPFN;
#else
	return map.fptmf3.fPFN;
#endif
    } else {
	union {
	...
	for (i = 0; i < index + 10; i++) {
		if (((size_t)(*u.vtablep)[i] & 0x1f0000) == 0x1f0000)
			panic("XXXPANICXXX: idx %d got %p\n", i, 
			    (*u.vtablep)[i]);
	}
#endif
#if 0
	panic("XXXPANICXXX: returning %p (%d)\n", (*u.vtablep)[index], index);
	panic("XXXPANICXXX: +1 would be %p (%d)\n", 
	    (*u.vtablep)[index + 1], index + 1);
#endif
	return ((*u.vtablep)[index]);
#endif
    }
}
