...
__cxa_pure_virtual( void )
{
	...
	for (i = 0; i < 20; i++)
		printf("%s: stack trace <%p> : %p\n", __FUNCTION__, 
		    &stack[i], stack[i]);

	printf("%s: continuing dizzily ...\n");
}
#endif
#else
...
#ifdef L4IOKIT
extern void *ctorstart(void);
extern void *ctorend(void);
#endif
...
#ifdef L4IOKIT
#ifdef VERBOSE_IOKIT
    printf("L4IOKit: %s modified called\n", __FUNCTION__);
#endif
#else
...
#ifdef L4IOKIT
#ifdef VERBOSE_IOKIT
    printf("L4IOKIt: %s modified and called\n", __FUNCTION__);
#endif
#else
    if (ki && ki->address) {
...
#ifdef L4IOKIT
#ifdef VERBOSE_IOKIT
    printf("%s: called.  ki %p ki->name %s ki->address %p", 
        __func__, ki, ki->name, ki->address);
#endif
#endif
    if (!ki || !ki->address)
...
#ifdef L4IOKIT
#ifdef VERBOSE_IOKIT
    printf("libkern: running constructors\n");
#endif

   {
#ifdef __APPLE_CC__
// XXX: -gl
// get rid of me soon, should use libkern original code
	int ctorstart, ctorend;
	{
		...
		for (i = 0; i < mh->ncmds; i++) {
			...
			for (j = 0; j < segcmd->nsects; j++) {
				if (strcmp(sect->sectname, 
				    "__constructor") == 0) {
					ctorstart = sect->addr;
					ctorend = ctorstart + sect->size;
					goto found;
				}
				sect++;
			}
...
found:
	int i;
	int ctorssiz = ((unsigned long)ctorend - (unsigned long)ctorstart);
	int nctors = ctorssiz / sizeof(structor_t);
	structor_t *structors = (structor_t *)ctorstart;
#ifdef VERBOSE_IOKIT
	printf("ctors at %p, ctorssiz = %d\n", ctorstart, ctorssiz);
#endif
	for (i = 0; i < nctors; i++) {

		if (structors[i]) {
			//printf("libkern: loading ctor %d at %p\n", 
			//    i, structors[i]);
			(*structors[i])();
		}
	}
...
#else
#ifdef __ELF__
    KMOD_INFO_NAME.address = 0xdecafbad;	/* catch this */
#elif defined(__APPLE_CC__)
    // XXX: should use generic libkern Mach-O code
    extern struct mach_header _mh_execute_header;
    KMOD_INFO_NAME.address = &_mh_execute_header;
#else
#error "unknown compiler"	/* nope. */
#endif
#endif
