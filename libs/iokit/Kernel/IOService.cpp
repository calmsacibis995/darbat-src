...
#ifdef L4IOKIT
    // for now.  I don't know why it sometimes it does not release itself ...
    // maybe it's due to the fact that our c++ constructors are not being
    // properly initialized??  For now we avoid the sleep before the crash
    // XXX FIXME!!!!!
    //L4_Sleep(L4_Never);
#endif
    self->release();
...
#ifdef L4IOKIT
    //IOLog("IOService::resolveInterrupt interruptSources %p\n", interruptSources);
#endif
...
#ifdef L4IOKIT
  //IOLog("IOService::resolveInterrupt interruptControllerName is %p\n", interruptControllerName);
#endif
...
#ifdef L4IOKIT
  //IOLog("IOService::resolveInterrupt interruptController %p\n", interruptController);
#endif
...
#ifdef L4IOKIT
  //IOLog("IOService::resolveInterrupt returns kIOReturnSuccess!\n");
#endif 
  return kIOReturnSuccess;
}
...
IOReturn IOService::lookupInterrupt(int source, bool resolve, IOInterruptController **interruptController)
{
...
#ifdef L4IOKIT
    //IOLog("IOService::lookupInterrupt _interruptSources == 0 && ret = %d\n", ret);
    //L4_KDB_Enter("bla");
#endif
...
#ifdef L4IOKIT
  //IOLog("IOService::lookupInterrupt source = %d, _numInterruptSources = %d\n", source, _numInterruptSources);
#endif 
  ...
  *interruptController = _interruptSources[source].interruptController;
...
#ifdef L4IOKIT
    //IOLog("*interruptController is %p\n", *interruptController);
    //L4_KDB_Enter("bla");
#endif
  ...
  if (*interruptController == NULL) {
    ...
    ret = resolveInterrupt(this, source);
#ifdef L4IOKIT
    //IOLog("resolveInterrupt ret is %d\n", ret);
    //L4_KDB_Enter("bla");
#endif
    if (ret != kIOReturnSuccess) return ret;    
...
#ifdef L4IOKIT
  //IOLog("IOService::registerInterrupt called \n");
  //L4_KDB_Enter("blah");
#endif
  ret = lookupInterrupt(source, true, &interruptController);
#ifdef L4IOKIT
  //IOLog("registerInterrupt: lookupInterrrupt returned 0x%lx, "
	//"interruptController is %p!\n", ret, interruptController);
  //L4_KDB_Enter("lookupInterrupt");
#endif
  if (ret != kIOReturnSuccess) return ret;
...
#ifdef L4IOKIT	/*ABI change, thanks Apple */
#ifdef __ppc__
