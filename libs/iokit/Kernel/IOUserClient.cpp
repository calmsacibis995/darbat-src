...
#ifdef L4IOKIT
// these are probably bogus, there is no way to tell what they
// do.  We will have to wait for the x86 Tiger sources to come out
// (when are they coming out, anyway?) to tell us.
bool IOUserClient::init()
{
	bool ok;

	IOLog("warning: IOUserClient::%s self-implemented function called\n",
	    __func__);
	ok = super::init();
	return (ok);
}

bool IOUserClient::init(OSDictionary * properties)
{
	bool ok;
