			       ...
			       task_t	    inTask)
{
    ...
    {
#ifdef L4IOKIT
	IOLog("warning: %s options & kIOMemoryPageable\n", __func__);
#endif
...
#else
	// XXX HACK: stuff it: just allocate 1-1 memory
	_buffer = IOMallocContiguous(capacity, alignment, 0);
#endif
...
#ifdef L4IOKIT
    //IOLog("IOBufferMemoryDesciptor _singleRange _buffer %p (%p) length %d  "
//	"alignemnt 0x%lx\n", _buffer, &_buffer, capacity, alignment);
#endif
...
void IOBufferMemoryDescriptor::free()
{
    ...
    * XXX: -gl
    *
    * As far as I know, we only allocate IOBufferMemoryDescriptor using
    * IOMallocContiguous which gives us 1-1 memory.
    *
    * So free that _only_ using that.
    */
    IOFreeContiguous(buffer, size);
#endif
}
