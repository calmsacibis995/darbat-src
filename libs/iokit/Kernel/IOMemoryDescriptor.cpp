...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
#ifdef L4IOKIT
IOReturn IOMemoryDescriptor::dmaCommandOperation(DMACommandOps op, void *vData, UInt dataSize) const
{
	IOLog("IOMemoryDescriptor::%s called\n", __func__);
	L4_KDB_Enter("shit");
}
#endif
...
{
	IOLog("IOGeneralMemoryDescriptor::%s called\n", __func__);
	L4_KDB_Enter("shit");
}
#endif
...
#ifdef L4IOKIT
    //IOLog("IOGeneralMemoryDescriptor::%s kernelPage %p\n",
//	__FUNCTION__, kernelPage);
#endif
                                           ...
                                           IOMapper *	mapper)
{
...
#ifdef L4IOKIT
    //IOLog("IOGeneralMemoryDescriptor::initWithOptions running!\n");
#endif
...
#ifdef L4IOKIT
	//IOLog("kIOMemoryTypePersistentMD set\n");
#endif
...
#ifdef L4IOKIT
	//IOLog("IOGeneralMemoryDescriptor::initWithOptions singlerange buffer "
		//"%p \n", buffers);
#endif
    ...
    if (_initialized) {
#ifdef L4IOKIT
	//IOLog("IOGeneralMemoryDescriptior::initWithOptinons "
	  //   "initailized set??\n");
#endif
        /*
...
#ifdef L4IOKIT
	//IOLog("IOGeneralMemoryDescriptor pageList is %p\n", pageList);
#endif
...
#ifdef L4IOKIT
	//    IOLog("IOGeneralMemoryDescriptor::initWithOptions buffers %p\n",
		//buffers);
#endif
...
#ifdef L4IOKIT
#if 0
//	    IOLog("IOGeneralMemoryDescriptor::initWithOptions IONew %p\n",			_ranges.v);
#endif
#endif
	    if (!_ranges.v)
                return false;
...
#ifdef L4IOKIT
#if 0
	    IOLog("getAddrLenForInd called in loop\n");
#endif
#endif
...
#ifdef L4IOKIT
#if 0
	IOLog("IOGeneralMemoryDescriptor::%s _length %lu _pages %lu "
	    "_rangesCount = %lu\n", 
	    __FUNCTION__, _length, _pages, _rangesCount);
#endif
#endif
    return true;
}
...
#ifdef L4IOKIT
#if 0
	IOLog("IOMemoryDescriptor::%s remaining %d dstAddr64 0x%llx "
	    "dstLen %lu\n",
	    __FUNCTION__,
	    remaining,
	    dstAddr64,
...
#ifdef L4IOKIT
#if 0
	    IOLog("%s::%s kIOMemoryTypePhysical, looking ...\n", 
		"IOGeneralMemoryDescriptor", __FUNCTION__);
#endif
#endif
...
#endif
#ifdef L4IOKIT	/* XXX WE don't have UPLs. */
	/*
	 * XXX we map it one to one with the special hack, so we can do this
	 * XXX FIXME!!!
	 * XXX PLEASE!!!
 	 */
	if (_singleRange.v.address) {
		address = _singleRange.v.address + offset;
		length = _singleRange.v.length - offset;
	} else {
		address = _ranges.v[0].address + offset;
		length = _ranges.v[0].length - offset;
	}
#if 0
	IOLog("IOGeneralMemoryDescriptor::%s offset %d will return "
...
#ifdef L4IOKIT
#if 0
    IOLog("IOMemoryDescriptor::%s looking ...\n", __FUNCTION__);
#endif

   // we only handle 32bit addresses for now, no 32bit pci window..  That's ok.
   phys32 = getPhysicalSegment(offset, lengthOfSegment);

#if 0 
   IOLog("IOMemoryDescriptor::%s got phys32 addr 0x%lx, len = %d\n",
	__FUNCTION__,
	phys32, (lengthOfSegment) ? *lengthOfSegment : 0);
   return (addr64_t)phys32;
#endif
#endif
#ifndef L4IOKIT
    phys32 = getPhysicalSegment(offset, lengthOfSegment);
...
#ifdef L4IOKIT
#if 0
	IOLog("Calling getAddrLenForInd 0x%lx 0x%lx 0x%lx 0x%lx 0x%lx\n",
	    startPage, numBytes, type, vec, range);
#endif
#endif
...
#ifdef L4IOKIT
#if 0
		IOLog("IOGeneralMemoryDesscriptor::%s kernelStart %p\n",
			__FUNCTION__, kernelStart);
#endif
#endif
...
#ifdef L4IOKIT
#if 0
    IOLog("IOMemoryDescriptor::initWithDescriptor: "
	"_memory = 0x%lx intoTask = 0x%lx _offset + _length = 0x%lx "
	"_memory->getLength() = 0x%lx redir = %d\n", 
	_memory, intoTask, _offset + _length, _memory ? _memory->getLength() :
	    0, redir);
#endif
#endif
...
#ifdef L4IOKIT
	if (!addressMap)
		addressMap = kernel_map;	/* XXX L4IOKIT HACK */
#endif
	if( !addressMap)
	    return( false);
...
#ifdef L4IOKIT
	ok = (kIOReturnSuccess == 
	    (myReturn = _memory->doMap( addressMap, &toAddress,
...
#ifdef L4IOKIT
#if 1
	IOLog("IOMemoryDescriptor::initWithDescriptor ok = %d myReturn = %d\n",
	    ok, myReturn);
#endif
#endif
    if (ok || redir)
	...
	IOByteCount		length )
{
	...
	if ((kIOMapReference|kIOMapUnique) == ((kIOMapReference|kIOMapUnique) & options))
	{
	    ...
	    {
#ifdef L4IOKIT
		IOLog("doMap err = kIOReturnNotReadable\n");
#endif
	...
	else
	{
...
#ifdef L4IOKIT
		IOLog("doMap err = kIOReturnVMError\n");
#endif
		    continue;
		}
	    }
	    ...
	    if( ref.sharedMem && (addressMap == kernel_map) && (kIOMemoryBufferPageable & _flags)) {
		err = IOIteratePageableMaps( ref.size, &IOMemoryDescriptorMapAlloc, &ref );
		IOLog("IOIteratePageableMaps err = %d\n", err); 
	    } else {
		err = IOMemoryDescriptorMapAlloc( addressMap, &ref );
		IOLog("IOMemoryDescriptorMapAlloc err = %d\n", err);
	    }
#endif
	}
...
#ifdef L4IOKIT
#if 0
	IOLog("IOMemoryDescriptor::doUnmap logical %p\n", logical);
#endif
#endif
...
IOVirtualAddress _IOMemoryMap::getVirtualAddress()
{
     ...
     * XXX HACK!
     *
     * HAH!  virtual addresses?  What virtual address?  :-)
     */
#if 0
    IOVirtualAddress _x = getPhysicalSegment(0, 0);
    IOLog("getVirtualAddress hacked to return phys = 0x%lx\n", _x);
    return _x;
#endif
    return( logical);
#endif
...
#ifdef L4IOKIT
    //IOLog("IOMemoryDescriptor::copyCompatible got past initial tests!\n");
#endif
...
#ifdef L4IOKIT
    //IOLog("IOMemoryDescriptor::copyCompatible got past ALL tests!\n");
#endif
...
#ifdef L4IOKIT
    //IOLog("IOMemoryDescriptor::setMapping newMap = %p\n", newMap);
#endif
    if( newMap
	...
	IOByteCount		length )
{
	...
	{
#ifdef L4IOKIT
	    //IOLog("IOMemoryDescriptor::makeMapping, in else...\n");
#endif
	    // look for an existing mapping
	    if( (iter = OSCollectionIterator::withCollection( _mappings))) {
   
#ifdef L4IOKIT
		//IOLog("IOMemoryDescriptor::makingMapping got iter\n");
#endif 
		while( (mapping = (_IOMemoryMap *) iter->getNextObject())) {
    
#ifdef L4IOKIT
		    //IOLog("IOMemoryDescriptor::makeMapping got map 0x%lx\n",
			// mapping);
#endif
			...
			IOLog("IOMemoryDescriptor::makeMapping copyCompatible "
			    "failed\n");
#endif
		}
		iter->release();
...
#ifdef L4IOKIT
IOReturn IOSubMemoryDescriptor::dmaCommandOperation(DMACommandOps op, void *vData, UInt dataSize) const
{
	IOLog("IOSubMemoryDescriptor::%s called.\n", __func__);
	L4_KDB_Enter("shit");
}
#endif
