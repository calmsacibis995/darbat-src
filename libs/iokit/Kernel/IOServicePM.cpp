...
#else
		/*
		 * Unlock first, and then start the ack timer.
		 * basically, it could be that we've been scheduled
		 * and then we could deadlock
		 *
		 * but could also be a bug in my bad implementation of
		 * thread_call
		 */
		    IOUnlock(priv->our_lock);
		    start_ack_timer();
