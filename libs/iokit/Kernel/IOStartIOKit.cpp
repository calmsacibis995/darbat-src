...
#ifdef L4IOKIT
extern	lck_mtx_t *threadArgMutex;
#endif
...
#ifdef L4IOKIT
    threadArgMutex = lck_mtx_alloc_init(NULL/*XXX*/, NULL/*XXX*/);
#endif
...
#ifdef VERBOSE_IOKIT
#ifdef L4IOKIT
    gIOKitDebug = (kIOLogSynchronous|kIOLogConfig|kIOLogMatch);
    gIOKitDebug |= ( kIOLogCatalogue|kIOLogYield|kIOLogProbe|kIOLogStart);
#endif
#else
	gIOKitDebug = kIOLogSynchronous;
#endif
...
#else
	int rc;
        if (KERN_SUCCESS != (rc = kmod_create_fake(kmodName->getCStringNoCopy(), vers))) {
	    printf("kmod_create_fake returned with rc %d\n", rc);
#endif
