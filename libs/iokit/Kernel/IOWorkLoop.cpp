...
#ifdef L4IOKIT	// runEventSources(): abi change
...
#ifdef L4IOKIT
	commandGate(this, OSMemberFunctionCast(IOCommandGate::Action, this,
...
#else
		/*
		 * Get rid of continuation as under L4 I/O Kit it requires
		 * a syscall (ExRegs) to restart with right SP and IP
		 * Alternative is to send the continuation argument and
		 * the restart instruction pointer to the scheduling server,  
		 * but that is 2 words worth of information ...
		 */
