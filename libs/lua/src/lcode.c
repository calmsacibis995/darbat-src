...
void luaK_storevar (FuncState *fs, expdesc *var, expdesc *exp_stupid_gcc) {
  switch (var->k) {
    case VLOCAL: {
      freeexp(fs, exp_stupid_gcc);
      luaK_exp2reg(fs, exp_stupid_gcc, var->info);
      return;
    }
    case VUPVAL: {
      int e = luaK_exp2anyreg(fs, exp_stupid_gcc);
    ...
    case VINDEXED: {
      int e = luaK_exp2RK(fs, exp_stupid_gcc);
