...
#ifdef L4IOKIT
    IOLog("IOKeyboardMapper::%s _parsedMapping.numDefs %d delegate %d\n",
	__func__, _parsedMapping.numDefs, _delegate);
#endif
 ...
 * XXX IOKit bug XXX
 *
 * This is in reserved, check against it first
 */
#endif

#ifdef L4IOKIT
  if (_reserved && _ejectTimerEventSource) {
#else
...
#ifdef L4IOKIT 
  if (_reserved && _slowKeysTimerEventSource) {
#else
...
static inline unsigned int NextNum(NewMappingData *nmd)
{
    if (nmd->bp >= nmd->endPtr)
		return(0);
#ifndef L4IOKIT	/* XXX: fucking lvalues */
    if (nmd->shorts)
		return OSSwapBigToHostInt16(*((unsigned short *)nmd->bp)++);
#else
    if (nmd->shorts) {
		unsigned short *_blah = (unsigned short *)nmd->bp;
		/* (cast to unsigned short) */
		nmd->bp += sizeof(unsigned short);
		return OSSwapBigToHostInt16(*_blah);
    }
#endif
    else
#ifdef L4IOKIT 
	{
		unsigned char *_blah= (unsigned char *)nmd->bp;
		nmd->bp++;
		return (*_blah);
	}
#else
	                                 ...
	                                 NXParsedKeyMapping * parsedMapping) const
{
...
#else
	    if ((j = NextNum(&nmd)) >= NX_NUMMODIFIERS) {
		IOLog("j = NextNum(&nmd)) >= NX_NUMMODIFIERS\n");
		return false;
	    }
#endif
...
#else
		if ((l = NextNum(&nmd)) >= NX_NUMKEYCODES) {
		    IOLog("NX_NUMKEYCODES returning false\n");
		    return false;
		}
#endif
	...
	if ( numMods > NX_NUMSPECIALKEYS ) {
		IOLog("numMods > NX_NUMSPECIALKEYS\n");
	    return false;
	}
#else
	if ( numMods > NX_NUMSPECIALKEYS )
...
{
#ifdef L4IOKIT
	IOLog("IOHIKeyboardMapper::%s, this %p _reserved %p\n",
		__func__, this, _reserved);
#endif
	...
	/*
 	 * XXX IOKit bug XXX
	 *
	 * This shit is stored in reserved, however, IONew only gets memory
	 * after the keyboard mapper is initialized.  If it's false, it
	 * then you compare against memory location zero!
	 *
	 * Not fucking good
	 */
#endif
	...
	if (_offParamDict)
        ...
        if (_onFnParamDict)
#else
        if (_reserved && _onFnParamDict)
#endif
                _onFnParamDict->release();

#ifdef L4IOKIT
        if (_reserved && _stickyKeysMouseClickEventSource) {
#else
...
#ifdef L4IOKIT
        if (_reserved && _stickyKeysSetFnStateEventSource) {
#else
