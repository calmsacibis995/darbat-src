...
#ifdef L4IOKIT
#if 0
	IOLog("%s: called, gKeyboardReservedArray %p count %d\n",
		__func__, gKeyboardReservedArray, count);Q
#endif
#endif
        for(i=0; i<count; i++)
        {
			...
			IOLog("%s: service %p, retVal->service %p\n",
			    __func__, service, retVal->service);
	    }
#endif
#endif
            if ( (data = (OSData *)gKeyboardReservedArray->getObject(i)) &&
...
{
#ifdef L4IOKIT
  IOLog("IOHIKeyboard::init called.\n");
#endif
