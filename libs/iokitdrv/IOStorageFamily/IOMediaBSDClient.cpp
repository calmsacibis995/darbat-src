...
#ifdef L4IOKIT
#define	DEV_BSIZE       512	/* i386/param.h */
extern "C" int copyout(const void *kaddr, user_addr_t udaddr, size_t len);
#endif
...
bool IOMediaBSDClient::start(IOService * provider)
{
...
#ifdef L4IOKIT
    //IOLog("IOMediaBSDClient::%s trying to start\n", __FUNCTION__);
#endif
    ...
    bool r = createNodes(media);
#ifdef L4IOKIT
    //IOLog("IOMediaBSDClient::%s createNodes media returned result = %d.\n", 
	//__FUNCTION__, r);
#endif
	...
	L4_Msg_t		msg;
	UInt32			minorID;
	extern L4_ThreadId_t	iokitservtid;
	/*
	 * XXX
	 *
	 * OK, I probably suck to do it like this, but whatever, it 
	 * works for the time being.  We really should do proper IOBSD
	 * matching, but we don't.  Since that is somewhat incomplete.
	 */
        minorID = gIOMediaBSDClientGlobals.getMinors()->locate(getProvider());
	L4_MsgClear(&msg);
	L4_MsgAppendWord(&msg, (L4_Word_t)minorID);
	L4_MsgLoad(&msg);
	tag = L4_Send(iokitservtid);
	if (L4_IpcFailed(tag))
		panic("iokit notification failed.\n");
#endif
    return true;
}
...
#ifdef L4IOKIT
    //IOLog("IOMediaBSDClient::createNodes whole = %p %lu\n", whole, 
	//slicePathSize);
#endif
...
#ifdef L4IOKIT
    //IOLog("IOMediaBSDClient::createNodes anchor = %p\n", anchor);
#endif
...
#ifdef L4IOKIT	
    //IOLog("createNodes %s %lu %lu %lu\n", 
	//minors->getMinor(minorID)->name, anchorID, kMajor, minorID);
#endif
    //
...
#ifdef L4IOKIT
    //IOLog("IOMediaBSDClient::createNodes ok.\n");
#endif
    return true; // (success)
...
inline IOMemoryDescriptor * DKR_GET_BUFFER(dkr_t dkr, dkrtype_t dkrtype)
{
	...
	uio_getiov(uio, 0, &__x, &__y);
	return IOMemoryDescriptor::withAddress(		// (single-range)
		(vm_address_t)__x,
		(vm_size_t)__y,
		uio_rw(uio) == UIO_READ ? kIODirectionIn : kIODirectionOut,
		NULL);
#if 0
	/*
	 * XXX
	 *
	 * I don't know enough about the I/O Kit.  Let's just cop this
	 * cost for now.
	 * It is unnecessary, I know, so you don't have to tell me that.
	 */
	return IOBufferMemoryDescriptor::
		withCapacity(uio_resid(uio), 
			uio_rw(uio) == UIO_READ ? kIODirectionIn :
			kIODirectionOut);
#endif
#endif
    }
...
int dkreadwrite(dkr_t dkr, dkrtype_t dkrtype)
{
...
#ifdef L4IOKIT
	// sometimes the above gives us garbage.  i have no idea why (yet)
	// but this should not be hard to fix ... so i will elave this
	// dodgy workaround here for now
	if (mediaBlockSize != 512) {
		// surpress noise
		//IOLog("%s: REALLY weird mediaBlockSize, clip to 512\n",
			//__FUNCTION__);
		mediaBlockSize = 512;
	}
#endif
...
#ifdef L4IOKIT	
#if 0
    // copy from physical address back to virtual address
    // (OK this sucks, but ...)
    if (uio_rw(((dio_t)dkr)->uio) == UIO_WRITE)
	memcpy(buffer->getPhysicalSegment(0, 0),
    	((dio_t)dkr)->uio->uio_iovs.iovp[0].iov_base, 
		byteCount);
#endif
#endif
    ...
    // copy from physical address back to virtual address
    // (OK this sucks, but ...)
    if (uio_rw(((dio_t)dkr)->uio) == UIO_READ)
    	memcpy(((dio_t)dkr)->uio->uio_iovs.iovp[0].iov_base, 
