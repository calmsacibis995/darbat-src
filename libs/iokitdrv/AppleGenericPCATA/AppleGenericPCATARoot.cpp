...
bool  AppleGenericPCATARoot::start( IOService * provider )
{
    ...
    {
#ifdef L4IOKIT
	IOLog("AppleGenericPCATARoot::%s super::start returns false.\n",
	__FUNCTION__);
#endif
        return false;
    }
...
#ifdef L4IOKIT
    IOLog("AppleGenericPCATA::%s fChannels = %p\n", __FUNCTION__, fChannels);
#endif
    if (fChannels == 0)
    {
        return false;
...
#ifdef L4IOKIT
    IOLog("AppleGenericPCATA::%s fOpenChannels = %p\n", 
	__FUNCTION__, fOpenChannels);
#endif
    if ( fOpenChannels == 0 )
    {
        return false;
...
OSSet * AppleGenericPCATARoot::createATAChannels( void )
{
    ...
    do {
        ...
        for ( UInt32 channelID = 0; channelID < 2; channelID++ )
        {
            ...
            if ( nub )
            {
                ...
                {
#ifdef L4IOKIT
		    IOLog("AppleGenericPCATARoot::%s setting object.\n",
			__FUNCTION__);
#endif
    ...
    {
#ifdef L4IOKIT
	IOLog("AppleGenericPCATARoot::%s BADNESS.\n", __FUNCTION__);
#endif
        nubSet->release();
        nubSet = 0;
    }
