...
#ifdef L4IOKIT
    IOLog("IONetworkStack::%s returns true!\n", __FUNCTION__);
    //L4_KDB_Enter("");
#endif
    return true;
}
                                         ...
                                         IOService * service )
{
...
#ifdef L4IOKIT
    IOLog("gIONetworkStack = %p netif = %p \n", 
	gIONetworkStack, netif);
    //L4_KDB_Enter("");
#endif
    if ( gIONetworkStack == 0 )
        return false;
    ...
    do {
        ...
        if ( gIONetworkStack->containsInterface(netif) == true )
        {
            success = true;
#ifdef L4IOKIT
	IOLog("already registered, breaking\n");
	//L4_KDB_Enter("");
#endif
            break;
        }
...
#ifdef L4IOKIT
	{
		IOLog("could not do addInterface()\n");
		//L4_KDB_Enter("");
#endif
            break;
#ifdef L4IOKIT
...
#ifdef L4IOKIT
	{
		IOLog("could not do gIONetworkStack->attach(netif)!!\n");
		//L4_KDB_Enter("");
#endif
            break;
#ifdef L4IOKIT
...
#ifdef L4IOKIT
	IOLog("gIONetworkStack->_registerPrimaryInterace && isPrimary\n");
	//L4_KDB_Enter("");
#endif
            ...
            if ( gIONetworkStack->getRegisteredInterface(prefix, unit) == 0 )
            {
                ...
                {
#ifdef L4IOKIT
		IOLog("preRegisterInterface + complete\n");
		//L4_KDB_Enter("");
#endif
...
#ifdef L4IOKIT
    IOLog("IONetworkStack interfacePublished returns %d!\n", success);
    //L4_KDB_Enter("");
#endif
    return success;
}
...
{
#ifdef L4IOKIT
    IOLog("IONetworkStack::completeRegistration array %p isSync %d\n",
	array, isSync);
    //L4_KDB_Enter("");
#endif
    if ( isSync )
