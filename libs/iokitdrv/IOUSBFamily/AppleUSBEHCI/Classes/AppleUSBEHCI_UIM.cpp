...
AppleUSBEHCI::UIMCreateInterruptTransfer(IOUSBCommand* command)
{
...
#ifdef L4IOKIT	
    IOLog("%s[%p]::UIMCreateInterruptTransfer - adr=%d:%d cbp=%p:%lx br=%s cback=[%lx:%lx:%lx])\n", getName(), this,  
    ...
    IOLog("CreateInterrupttransfer buffer = 0x%lx\n", 
...
#ifdef L4IOKIT
    IOLog("AppleUSBEHCI::%s returning status %d\n", __FUNCTION__, status);
    //L4_KDB_Enter("");
#endif
    return status;
}
...
#ifdef L4IOKIT
		IOLog("dmaStartAddr (pBuffer) = 0x%lx\n", dmaStartAddr);
		//L4_KDB_Enter("");
#endif
