...
#ifdef L4IOKIT
#if 0
    IOLog("AppleUSBHub::%s request.pData = 0x%lx\n", __FUNCTION__,
	request.pData);
#endif
#endif
...
#ifdef L4IOKIT
#if 0
    IOLog("AppleUSBHub::%s status = %d bufferSizeRemaining = %d\n",
	__FUNCTION__, status, bufferSizeRemaining);
#endif
#endif 
    switch (status)
    {
...
#ifdef L4IOKIT
#if 0
	IOLog("AppleUSBHub::%s _hubIsDead = %d _workThread = %p\n",
	    __FUNCTION__, _hubIsDead, _workThread);
#endif
#endif
            if ( !_hubIsDead )
            {
...
#ifdef L4IOKIT
#if 0
    IOLog("AppleUSBHub::%s target = %p, me = %p\n",  __FUNCTION__, target, me);
#endif
#endif
    if (!me)
        return;
...
#ifdef L4IOKIT
#if 0
    IOLog("AppleUSBHub::%s %d %p %p\n", 
	__FUNCTION__, isInactive(), _buffer, _ports);
#endif
#endif
