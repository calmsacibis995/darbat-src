                               ...
                               IOUSBIsocFrame *	pFrames)
{
    ...
    commandGate->commandWakeup(syncTarget->flag,  true);
#ifdef L4IOKIT
    // we can do this because send is blocking for normal synch ipc
    // so if we can get here that means the wakeup succeeded
    IOLog("%s: commandGate woke\n", __FUNCTION__);
#endif
}
...
#ifdef L4IOKIT
	IOLog("creating root hub dev\n");
	//L4_KDB_Enter("");
#endif
        err = CreateRootHubDevice( provider, &_rootHubDevice );
#ifdef L4IOKIT
	IOLog("created root hub device\n");
	//L4_KDB_Enter("");
#endif
...
#ifdef L4IOKIT
	IOLog("rootHubDevice registered service\n");
	//L4_KDB_Enter("");
#endif
        return true;
...
#ifdef L4IOKIT
#if 0
   IOLog("%s[%p]::InterruptPacketHandler: addr %d:%d(%s) complete status=0x%x bufferSizeRemaining = %d (%d)\n",  me->getName(), me, command->GetAddress(), command->GetEndpoint(), command->GetDirection() == kUSBIn ? "in" : "out", status, (int)bufferSizeRemaining, command->GetReqCount());
#endif
#endif
...
#ifdef L4IOKIT
    USBLog(7,"%s[%p]::BulkPacketHandler: addr %d:%d(%s) complete status=0x%x bufferSizeRemaining = %d (%d)\n", me->getName(), me, command->GetAddress(), command->GetEndpoint(), command->GetDirection() == kUSBIn ? "in" : "out", status, (int)bufferSizeRemaining, command->GetReqCount());
#endif
...
#ifdef L4IOKIT
#if 0
    IOLog("%s: complete.action = %p param %p status %d actualByteCount = %d\n",
	__func__, completion.action, 
	completion.parameter, status, actualByteCount);
#endif
#endif
...
#ifdef L4IOKIT	
#if 0
	IOLog("%s[%p]::DeviceRequest [%x,%x],[%x,%x],[%x,%lx]\n",getName(),this, 
...
#ifdef L4IOKIT
#if 0
	IOLog("SetRequest req 0x%lx addr 0x%lx ep 0x%lx\n", 
	    request, address, ep);
#endif
#endif
...
#ifdef L4IOKIT
	IOLog("SetRequest with req = 0x%lx\n", request);
#endif
