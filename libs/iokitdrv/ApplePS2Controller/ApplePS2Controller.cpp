...
{
#ifdef L4IOKIT
    IOLog("interruptHandlerKeyboard: CALLED _irqcount = %d!\n", _irqcount++);
#endif
...
#ifdef L4IOKIT
  IOLog("interruptHandlerKeyboard: waiting for kOutputReady\n");
#endif
  if ( ((status = inb(kCommandPort)) & kOutputReady) )
  {
    ...
    {
#ifdef L4IOKIT
  IOLog("interruptHandlerKeyboard: is mouse (WTF?)\n");
#endif
...
#ifdef L4IOKIT
  IOLog("interruptHandlerKeyboard: is keyboard, key is 0x%lx\n", key);
#endif
...
#ifdef L4IOKIT
  IOLog("ApplePS2Controller::installInterruptAction: installing. %d %p %p %d\n",
    deviceType, target, action, _interruptInstalledKeyboard);
#endif
...
#ifdef L4IOKIT
  IOLog("ApplePS2Controller::%s just before interruptOccurred\n", __FUNCTION__);
#endif
...
#ifdef L4IOKIT
  IOLog("ApplePS2Controller::interruptOccurred: OMG holy shit hardwareOffline "
    "is %d!!\n", _hardwareOffline);
#endif
...
#ifdef L4IOKIT
  IOLog("ApplePS2Controller::%s in processRequest\n", __FUNCTION__);
#endif
  UInt8         byte;
...
hardware_offline:
  ...
  {
#ifdef L4IOKIT
    IOLog("ApplePS2Controller::%s running completionAction\n", __FUNCTION__);
#endif
...
#ifdef L4IOKIT
  IOLog("ApplePS2Controller::%s nprocessRequestQueue\n", __FUNCTION__);
#endif
...
{
  IOLog("ApplePS2Controller::%s running\n", __FUNCTION__);
