...
#ifdef L4IOKIT
#if 0
	IOLog("IOATABlockStorageDriver::%s caps = 0x%lx\n",
	    __FUNCTION__,
	    fDeviceIdentifyData[kATAIdentifyDriveCapabilities]);
#endif
	//L4_KDB_Enter("X");
#endif
...
IOATABlockStorageDriver::sValidateIdentifyData ( UInt8 * deviceIdentifyData )
{
	...
	if ( deviceIdentifyData[offset] == kChecksumValidCookie )
	{
		...
		IOLog("ATA Disk: not kChecksumValidCookie, not checking.\n");
		IOLog("ATA Disk: maybe you are running the simulator????\n");
	}
#endif
#endif
