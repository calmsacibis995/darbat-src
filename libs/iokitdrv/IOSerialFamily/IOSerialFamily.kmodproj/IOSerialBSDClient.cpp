...
#ifdef L4IOKIT
/* XXX crap that should belong elsewhere */
extern int serialReady;	/* XXX need notification of when serial is ready!! */
void *kernel_flock;	/* XXX: fixme */
#include <sys/param.h>	/* where most of the stuff are */
#include <l4/kdebug.h>
#endif
...
iossread(dev_t dev, struct uio *uio, int ioflag)
{
	...
	char buf[512];
	int i;
	rc = me->sessionDequeueData(sp, buf, 512, &crap, 1);
#if 0
	IOLog("sessionDequeueData = %d\n", rc);
#endif
...
iosswrite(dev_t dev, struct uio *uio, int ioflag)
{
	...
	UInt32 crap;
	IOReturn rc;
	// essentially ttwrite is a stub.
	// we just pipe it to serial.
	// XXX FIXME, PLEEZ? XXX
	rc = me->sessionRequestEvent(sp, PD_E_TXQ_AVAILABLE, &crap);
#if 0
	IOLog("sessionRequestEvent rc = %d\n", rc);
#endif
	// yes, we woudl normally grab the uios,
	// but here we don't.
	// XXX FIXME
	// see also iguana/iokit/src/iokit.c
	rc = me->sessionEnqueueData(sp, (char *)uio, 1, &crap, false);
	//IOLog("sessionEnqueueEvent rc = %d\n", rc);
	bsdld_start(tp);
    }
#endif
...
#ifdef L4IOKIT
    IOCreateThread(OSMemberFunctionCast(IOThreadFunc,
	this, &IOSerialBSDClient::rxFunc), this);
    IOCreateThread(OSMemberFunctionCast(IOThreadFunc,
	this, &IOSerialBSDClient::txFunc), this);
#else
