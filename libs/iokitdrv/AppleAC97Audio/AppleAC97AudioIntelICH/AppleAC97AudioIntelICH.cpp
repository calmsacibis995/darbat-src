...
#if 0
    /*
     * Removing the above for now, seems the code is broken (????)
     *
     * I can't figure out what it's trying to do, this is not the end of
     * the world, even if we don't ahve IOBufferMemoryDescriptor, it just
     * means that we don't get 6 channel audio (which would be bad, but 
     * whatever.)
     */
    buffer = NULL;
#endif
