...
#ifdef L4IOKIT
    //IOLog("AppleIntelPIIXATARoot::probe\n");
    //L4_KDB_Enter("X");
#endif
...
{
#ifdef L4IOKIT
    //IOLog("AppleIntelPIIXATARoot::start starting\n");
    //L4_KDB_Enter("X");
#endif
...
#ifdef L4IOKIT
    //IOLog("AppleIntelPIIXATARoot::start returning true!!\n");
    //L4_KDB_Enter("X");
#endif
    ...
    do {
        ...
        if ( getProperty( kSerialATAKey ) == kOSBooleanTrue )
        {
            ...
            if (hwName)
            {
                ...
                hwName->isEqualTo("ICH7-M SATA"))
#else
                else if (hwName->isEqualTo("ICH6-M SATA"))
#endif
                {
#ifdef L4IOKIT
		   // L4IOKit hack: put this thing into non-ahci mode
		   if (hwName->isEqualTo("ICH7-M SATA")) {
		   IOLog("Map value = 0x%lx\n", mapValue);
			/* clear the top two bits for ide operation */
			UInt8 _mapValue = mapValue & 63;
			IOLog("Map Value is now 0x%lx\n", _mapValue);
			_provider->configWrite8(kPIIX_PCI_MAP, _mapValue);
            		setProperty( kPortMappingKey, _mapValue, 8 );
		   }
#endif
                    mapValue &= 0x3;
