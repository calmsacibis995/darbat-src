...
#else
    if (reserved == 0) {
	IOLog("IOPCIBridge::%s IONewFailed\n", __FUNCTION__);
	return false;
    }
#endif
...
#ifdef L4IOKIT
    bool r = configure(provider);
    if (!r) {
	IOLog("IOPCIBridge::configure failed, return r=%d!\n", r);
	return false;
    }
#else
...
#ifdef L4IOKIT
    //IOLog("%s::%s matches is %d\n", getName(), __FUNCTION__, matches);
    //L4_KDB_Enter("X");
#endif
    return (matches);
}
...
{
#ifdef L4IOKIT
    //IOLog("IOPCI2PCIBridge::probe running\n");
#endif
...
#ifdef L4IOKIT
    //IOLog("IOPCI2PCIBridge::configure\n");
#endif
    if (end)
    {
        start = (end & 0xfff0) << 16;
...
#else
    bool r = (super::configure(provider));
    //IOLog("IOPCI2PCIBridge::%s super::configure returned %d.\n", 
//	__FUNCTION__, r);
  //  L4_KDB_Enter("x");
    return r;
#endif
...
#if defined(L4IOKIT) && defined(__EFI__)
    printf( "XXXX: trying to CPR RPCI, dn = %d, pin = %d (%s)\n",
	    nub->space.s.deviceNum,
	    pin, nub->getName() );

    /* XXXX */
	/* 
	 * XXX don't route the stuff that we do not need.  
	 * XXX this means we will get interrupts on the dev (esp. the
	 * XXX case with firewire.  And we end up disabling the intr.
	 */
/* XXX: bus num == 2 for iMac, 1 for mac mini */
/* XXX: on my iMac lspci dump it says the intr is 16.  Maybe different
 * XXX: mac mini??
 */
    if( nub->space.s.busNum == 2 && nub->space.s.deviceNum ==  0 && pin == 0 )
    {
#if 1
	    /* seems to be same as linux - don't know if PEX is routable? */
	    IOLog("Found network setting IRQ 16\n");
	    IOLog("XXXX: using wrong intr for network!\n" );
	    nub->configWrite8(kIOPCIConfigInterruptLine, 16);
#else
	    /* CEG ICH7 hand xlation (wrong!) */
	    IOLog("Found network setting IRQ 17\n");
	    nub->configWrite8(kIOPCIConfigInterruptLine, 17);
#endif
    }
#if 0
    if (nub->space.s.deviceNum == 0x3 && pin == 0) {
	    IOLog("Found firewire setting IRQ 18\n");
	    nub->configWrite8(kIOPCIConfigInterruptLine, 18);
    }
#endif
    if (nub->space.s.deviceNum == 27 && pin == 0) {
	    IOLog("Found Audio setting IRQ 21\n");
	    nub->configWrite8(kIOPCIConfigInterruptLine, 21);
    }

    if( nub->space.s.deviceNum == 31 && pin == 0 )
    {
	    printf( "Found IDE - setting 19\n" );
	    nub->configWrite8( kIOPCIConfigInterruptLine, 19 );
    }

    if( nub->space.s.deviceNum == 31 && nub->space.s.functionNum == 2 && pin == 1 )
    {
	/* XXX on AHCI systems it indicates that it is 18.  However,
	 * when we fiddle with the port mapping register on the PIIX 
	 * chips to put it into legacy mode, it becomes 14.  We don't
	 * even bloody know if it is edge or level (should be edge if
	 * you are on 14, otherwise, level.  Of course, we treat edge
	 * and level the same way, so maybe it won't make so much of
	 * a difference to us (??)  but then 18 seems to work anyway 
	 * for the first channel (?????)
	 */
	    printf( "Found SATA - setting 18, er, 19\n" );
	    // nub->configWrite8( kIOPCIConfigInterruptLine, 18 );
	    nub->configWrite8( kIOPCIConfigInterruptLine, 19 );
    }

    if( nub->space.s.deviceNum == 29 )
    {
	    irq = 0;
#if 0
	    if( pin == 0 )
	    ...
	    if( pin == 3 )
		    irq = 16;
#endif
	    printf( "Found USB - setting %d\n", irq );
	    nub->configWrite8( kIOPCIConfigInterruptLine, irq );
    }
#endif	/* L4IOKIT && EFI */
	...
	printf("rPCI: Resolved interrupt %ld (%d) for %s\n",
	       irq, nub->configRead8(kIOPCIConfigInterruptLine),
	       nub->getName());
...
/*chuck is an idiot for forgetting to comment this line out*/
#if defined(L4IOKIT) && !defined(__EFI__)
